...
void QUICreator::initForm()
{
    ...
    this->initTreeWidget();
    this->initListWidget();
    this->initListWidget_2();
    this->initOther();
    this->initStyle();
    this->initTranslator();
    this->initTreeWidget_2();
...
//    int width =500;
    int width = QApplication::desktop()->availableGeometry().width() - 120;
    ...
    ui->tableWidget->setColumnWidth(4, width * 0.4);
...
//    ui->treeWidget_2->setHeaderLabel(" 树状列表控件");
...
void QUICreator::on_importFile_clicked()
{
    QString filter = "所有文件 (*.*)";
    ui->path->setText(myHelper::GetFileName(filter));

}

void QUICreator::on_training_clicked()
{
